...
function Page({user,onLogout}){const [controls,setControls]=useState([]),[control,setControl]=useState(null),[tags,setTags]=useState([]),[members,setMembers]=useState([]),[transactions,setTransactions]=useState([]); const refresh=useCallback(()=>control&&Promise.all([api(`/controls/${control.control_id}/tags`),api(`/controls/${control.control_id}/members`),api(`/controls/${control.control_id}/transactions?page=1&page_size=20`)]).then(([a,b,c])=>{setTags(a);setMembers(b);setTransactions(c.items)}),[control]); useEffect(()=>{api("/controls").then(x=>{setControls(x);setControl(x[0]||null)});},[]); useEffect(()=>{if(control)refresh()},[control,refresh]); const currentRole=members.find(member=>member.user_id===user.user_id)?.role;const canEdit=currentRole==="owner"||currentRole==="editor";const isOwner=currentRole==="owner";const path=window.location.pathname; if(!control)return <Shell {...{user,onLogout,control,setControl,controls}}><div className="empty-page"><WalletCards size={40}/><h2>Comece seu controle financeiro</h2><p>Crie seu primeiro controle para acompanhar entradas, saídas e objetivos.</p><button data-testid="create-control-button" className="primary" onClick={async()=>{const name=prompt("Nome do controle");if(name){const c=await api("/controls",{method:"POST",data:{name,description:""}});setControls([c]);setControl(c)}}}><Plus/> Criar controle</button></div></Shell>; let view=<Dashboard {...{control,tags,members,transactions,onRefresh:refresh,canEdit,isOwner}}/>; if(path==="/lancamentos")view=<TransactionsPage control={control} tags={tags} transactions={transactions} onRefresh={refresh} user={user} members={members}/>; if(path==="/tags")view=<TagsPage control={control} tags={tags} onRefresh={refresh} canEdit={canEdit}/>; if(path==="/membros")view=<MembersPage control={control} members={members} user={user} onRefresh={refresh} onDeleteControl={async()=>{if(!window.confirm("Excluir este controle e todos os seus dados?"))return;try{await api("/controls/"+control.control_id,{method:"DELETE"});const next=await api("/controls");setControls(next);setControl(next[0]||null)}catch(error){alert(error.response?.data?.detail||"Não foi possível excluir o controle.")}}}/>; return <Shell {...{user,onLogout,control,setControl,controls}}>{view}<AiInsights key={control.control_id} control={control}/></Shell>}
function AppRouter(){const loc=useLocation(),[user,setUser]=useState(null),[loading,setLoading]=useState(true); useEffect(()=>{if(loc.hash.includes("session_id="))return;api("/auth/me").then(setUser).catch(()=>{}).finally(()=>setLoading(false))},[loc.hash]); if(loc.hash.includes("session_id="))return <AuthCallback/>; if(loading)return <main className="auth-page"><div className="loader"/></main>; if(!user)return <Login onLogin={setUser}/>; return <Page user={user} onLogout={()=>api("/auth/logout",{method:"POST"}).then(()=>setUser(null))}/>}
function AiInsights({control}){const [text,setText]=useState(""),[loading,setLoading]=useState(false),[error,setError]=useState("");const generate=async()=>{setLoading(true);setText("");setError("");try{const response=await fetch(`${API}/controls/${control.control_id}/ai/insights`,{method:"POST",credentials:"include"});if(!response.ok)throw new Error();const reader=response.body.getReader(),decoder=new TextDecoder();let buffer="";while(true){const {value,done}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});const parts=buffer.split("\n\n");buffer=parts.pop();for(const line of parts){const raw=line.replace(/^data:\s*/,"");if(!raw)continue;const event=JSON.parse(raw);if(event.text)setText(prev=>prev+event.text);if(event.error)setError(event.error)}}}catch(e){setError("Não foi possível gerar os insights agora.")}finally{setLoading(false)}};return <section className="ai-panel"><div className="ai-heading"><div><span className="eyebrow">Assistente financeiro</span><h3>Insights com Gemini</h3><p className="muted">Análise baseada apenas nos seus totais agregados.</p></div><button data-testid="generate-ai-insights-button" className="primary" onClick={generate} disabled={loading}>{loading?"Analisando…":"Gerar insights"}</button></div>{text&&<p data-testid="ai-insights-result" className="ai-result">{text}</p>}{error&&<div data-testid="ai-insights-error" className="error">{error}</div>}</section>}
